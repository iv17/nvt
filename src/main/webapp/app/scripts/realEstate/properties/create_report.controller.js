(function() {
	...
	angular
		.module('nvtApp')
		.controller('CreateReportController', ['$scope', '$rootScope', '$state', '_', 'ReportResource',
		 'RealEstateResource', '$stateParams', '$log', '$window','toastr',  '$localStorage',
			function($scope, $rootScope, $state, _, ReportResource, RealEstateResource, $stateParams, $log, $window,
				toastr, $localStorage) {

				var realEstateId = $stateParams.realEstateId;

				var createReportRequest = {   // I NA SPRINGU OVAKAV OBJEKAT ZA PREUZIMANJE PARAMETARA
      		id : realEstateId
				};

				ReportResource.create(createReportRequest)
				.then(function(items) {
          $scope.realEstate = items.realEstate;
					$scope.realEstateComments = items.comments;
					toastr.success('Prijavili ste nekretninu kao neprikladnu!');
				})
