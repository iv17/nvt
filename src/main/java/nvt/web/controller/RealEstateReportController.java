...
public class RealEstateReportController {
	...
	public ResponseEntity<RealEstateReportDTO> createReport(@RequestBody RealEstateReportDTO realEstateReportDTO) {
		...
		RealEstate realEstate = realEstateService.findById(realEstateReportDTO.getRealEstateDTO().getId());
		if(realEstate == null) {

		}
		realEstateReport.setRealEstate(realEstate);

		User user = userService.findById(realEstateReportDTO.getUserDTO().getId());
		if(user == null) {
	...
	//@RequestMapping(method = RequestMethod.POST, consumes = "application/json")
	public ResponseEntity<RealEstateReportDTO> saveRealEstateReport(@RequestBody RealEstateReportDTO realEstateReportDTO) {
		...
		return new ResponseEntity<RealEstateReportDTO>(newRealEstateReportDTO, HttpStatus.CREATED);
	}


	@RequestMapping(method = RequestMethod.PUT, consumes = "application/json")
	public ResponseEntity<RealEstateReportDTO> updateRealEstateReport(@RequestBody RealEstateReportDTO realEstateReportDTO) {

		RealEstateReport realEstateReport = realEstateReportService.findById(realEstateReportDTO.getId());

		if(realEstateReport == null) {
			return new ResponseEntity<RealEstateReportDTO>(HttpStatus.BAD_REQUEST);
		}
		realEstateReport.setReport(realEstateReportDTO.getReport());
		realEstateReport.setPosted(realEstateReportDTO.getPosted());
	...
	public ResponseEntity<Void> deleteRealEstateReport(@PathVariable Integer id) {

		RealEstateReport realEstateReport = realEstateReportService.findById(id);
		if(realEstateReport == null) {
			return new ResponseEntity<Void>(HttpStatus.NOT_FOUND);
		}
		realEstateReportService.removeById(id);
